...
# TODO: logs
connection = psycopg2.connect(dbname="bot_database", host="database", user="bot", password="db_pass", port="db_port")
...
def find_file(file_name: str):
    cursor.execute(f"SELECT file_name, description"
                   f"FROM {table_name}"
...
def send_file(owner_id: int, file_name: str):
    cursor.execute(f"SELECT owner_id, file_id"
                   f"FROM {table_name}"
                   f"WHERE owner_id = %s AND file_name = %s",
                   (owner_id, file_name))
    response = cursor.fetchall()
